...
def evaluate_clf(type_list=None, y_test=None, y_pred=None, threshold=None):
    '''
...
def add_metrics_to_dict(r, yt, yp, type_list, threshold_list):
    '''
    ...
    for threshold in threshold_list:
        results = evaluate_clf(type_list=type_list, y_test=yt, y_pred=yp, threshold=threshold)
        for x in type_list:
            get_str = str(x) + " at "+str(threshold)
            if results[get_str]:
                r[get_str].append(results[get_str])
            else:
                r[get_str].append(0)     
    return r
